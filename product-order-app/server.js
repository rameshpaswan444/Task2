...
// Sample product and courier data
const products = [
...
const courierCharges = [
  { weightRange: '0-200g', charge: 5 },
  { weightRange: '200-500g', charge: 10 },
  { weightRange: '500-1000g', charge: 15 },
  { weightRange: '1000-5000g', charge: 20 },
];
...
app.post('/submit-order', (req, res) => {
  ...
  // Function to calculate total price and weight of selected items
  const { totalPrice, totalWeight } = calculateOrderDetails(selectedItems);

  // Function to divide items into packages based on specified rules
  const packages = divideIntoPackages(selectedItems);

  res.json({ packages, totalPrice, totalWeight });
});

// Helper function to calculate total price and weight
function calculateOrderDetails(selectedItems) {
  let totalPrice = 0;
  let totalWeight = 0;
...
// Helper function to divide items into packages
function divideIntoPackages(selectedItems) {
  // Implement package division logic here
  // Return packages in the required format
  return [];
}
